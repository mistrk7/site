...
If there has been one major movement to address the impermanence of platforms, it's called [decentralisation](https://en.wikipedia.org/wiki/Decentralization#In_technology_and_the_Internet). If you want digital independence then you should host your own site and content. Historically this means putting yourself back into the stone ages with self-hosting, slow as hell discovery and a divorce from social media as platforms demote all sorts of external links.  

Today however, decentralisation comes in many different forms. We've gone through the era of [Web 2.0](https://en.wikipedia.org/wiki/Web_2.0) where platforms emerged to allow us to **read** and **write** to their sites. [Web3](https://en.wikipedia.org/wiki/Web3), not to be confused with Web 3.0 *(stupid naming)* is meant to allow us to **read**, **write** and **share** information all together. 

This is where [Bluesky](https://en.wikipedia.org/wiki/Bluesky) and the [AT Protocol](https://en.wikipedia.org/wiki/AT_Protocol) comes in. Whilst not strictly advertised or associated with Web3 as much, probably due to its prior close association with blockchain technologies and NFTs, its definition and ethos aligns closely. It's all about having others share parts of information make up a platform through [distributed networks](https://en.wikipedia.org/wiki/Distributed_networking) instead of centralised ones. The AT Protocol defines a stack of interconnected servers and services designed to collectively host, aggregate and push users information. It's designed to hand control back to the users. Ultimately, they own it.  

I won't go too deep into how it works, but [this post by Paul Frazee](https://www.pfrazee.com/blog/practical-decentralization) does a good job at explaining it. It's [not entirely perfect](https://dustycloud.org/blog/how-decentralized-is-bluesky/) for some people. But the important part is that there's a popular social media platform using a decentralised distribution network that's user-friendly and rapidly growing, so why not join in on the fun? Even dedicated blogging platforms have been built on the same protocol such as [leaflet](https://leaflet.pub) and [pckt](https://pckt.blog). In the end, I'm able to stay digitally independent *and* discoverable in a progressive way. 
...
In April of 2026 I attended Revision, the biggest [Demoscene](https://en.wikipedia.org/wiki/Demoscene) demoparty going in Germany. Unfortunately, I didn't come with a *demo* prepared for the event so I focused on restructuring and refining my website instead to reflect what I was also doing there which was recording and [documenting](https://www.youtube.com/@misterk-in) the event as a newcomer. There I met new friends, partied a bit, and developed the bulk of my new branding as part of misterk.in.
...
I'll leave you with an episode of [Homestar Runner](https://youtu.be/2z7kVH9xePM?si=hvYUD_mDVqwc1SOt). Thank you for reading :).